...
export function LibraryView({
  ...
  const { recent, loadingRecent, loadRecent, orphans, restoring, restoreOrphan } =
    useResultsStore()
          ...
          {anime.temporadas.map((t) => (
            <div key={t.numero} className="flex flex-col gap-1">
              ...
              <ul className="grid gap-1 sm:grid-cols-2">
                {t.episodios.map((ep) => (
                  <li key={ep.episodeId}>
                    <EpisodeRow ep={ep} onOpen={onOpen} />
                  </li>
                ))}
              </ul>
            </div>
...
function EpisodeRow({
  ...
  return (
    <button
      ...
      title={ep.animeTitle}
      className="flex w-full items-center gap-2 rounded-md border border-transparent bg-surface-sunken px-2.5 py-1.5 text-left transition-colors hover:border-border hover:bg-surface-hover"
    >
